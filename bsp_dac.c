...
/*const uint16_t Sine12bit[POINT_NUM] = {
	3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,3,4093,4093,4093,4093,4093,4093,4093,4093,4093,4093,4093,4093,4093,4093,4093,4093
};
*/
const uint16_t Aine12bit[POINT_NUM] = {
...
void DAC_TIM_Config(int a)
{
  ...
  /* TIM2通用定时器配置 */                                                          /* f=72M/((1+T_Prescaler)*（1+T_Period*32)）             */
																																										//定时周期 168  f=6700        28 f=40180   82-13700  505-2232
	if(a==0)TIM_TimeBaseStructure.TIM_Period = (504-1);       									
  else 
		if(a==1)TIM_TimeBaseStructure.TIM_Period = (168-1);       									
		else 
			if(a==2)TIM_TimeBaseStructure.TIM_Period = (82-1);
			else
				if(a==3)TIM_TimeBaseStructure.TIM_Period = (28-1);
...
void DAC_Mode_Init(int i)
{
	...
	if(i==-1)
		...
		{
			DualSine12bit[Idx] = (Aine12bit[Idx] << 16) + (Aine12bit[Idx]);
		}
	}
	else
	{		
